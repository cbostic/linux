...
// This version is used to get rid of the warning messages.
// Some functions call the macro by passsing the address of a parameter.
// Since the address will never be ZERO, the checking of the address will
// generate warning messages.
#define iic_ffdc_loc_1(f)
...
#ifdef IIC_DMA
    dma_key_t dma_key;			// for dma xfrs
#endif
...
struct iic_eng
{
    ...
    iic_trace_entry_t *trace;		//pointer to trace buffer
    uint64_t enabled;
};
...
void iic_trace(iic_eng_t* eng, char port, char flags, unsigned char addr,
		unsigned char read, unsigned long offset, char status,
		unsigned long size);
...
void iic_opb_set_slv_addr(iic_client_t * client, unsigned char addr);
void iic_opb_set_dev_width(iic_client_t * client, unsigned char width);
void iic_opb_reset(iic_client_t * client, unsigned long type);
iic_bus_t * iic_opb_get_bus(unsigned char bus_num);
#endif
