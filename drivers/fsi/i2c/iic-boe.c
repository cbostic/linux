...
#define FSPTRACE_FILE_ID TFID_IIC_BOE
...
//void iic_boe_get_ffdc(iic_eng_t* eng, iic_ffdc_t* element);
...
int iic_boe_use_dma(iic_xfr_t* xfr)
{
	...
	iic_xfr_opts_t* t_opts = &xfr->opts.xfr_opts;
	unsigned long flags = DMA_FSI | DMA_UBUF | DMA_FIFO | DMA_DEVICE_PACED | DMA_BYTE;

	IENTER();
	/* if IIC_FORCE_DMA is set, we ignore the size. */
	if(((xfr->size < IIC_BOE_MIN_DMA_SIZE) &&
	   !(IIC_FORCE_DMA & t_opts->flags)) ||
	   (IIC_NO_DMA & t_opts->flags))
	{
		goto exit;
	}

	/* xfrs that must be split up into smaller transfers are not
	 * allowed to use DMA (requires writing metadata to the fifo).
	 */
	if(t_opts->rsplit)
	{
		goto exit;
	...
	/* cannot use DMA for devices behind a cascaded FSI master engine */
	if(fsidevice_on_sublink(to_fsidevice(eng->dev)))
	{
		IDBGd(1, "No DMA for bus[%08lx] cmFSI\n", xfr->client->bus->bus_id);
		goto exit;
	}

	init_dma_key(&xfr->dma_key);
...
//			iic_ffdc_loc(&xfr->ffdc);
	...
	if(stat & IIC_BOE_S_CMD_COMP)
	{
		...
		/* Update bytes_xfrd if this was a DMA transfer */
		if(!test_bit(IIC_XFR_OFFSET_PHASE, &xfr->flags) &&
		    test_bit(IIC_XFR_DMA, &xfr->flags))
		{
			unsigned long bytes_xfrd = 0;
			rc = iic_readw(eng, IIC_BOE_CMD, &bytes_xfrd, &xfr->ffdc);
			if(rc)
				goto xfr_err;
			bytes_xfrd &= IIC_BOE_XFR_LEN;
			xfr->bytes_xfrd += bytes_xfrd;
		}
		...
		/* this is needed for when an offset was used */
		/* Don't enable the DATA REQUEST interrupt if using DMA */
		if(test_bit(IIC_XFR_DMA, &xfr->flags))
		{
			if(!test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags))
			{
				IFLDd(0, "submit dma\n");
				set_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags);
#ifdef IIC_DMA
				rc = dma_submit_aio(&xfr->dma_key,
						iic_boe_dma_callback,
						xfr);
				if(rc)
				{
//					iic_fill_xfr_ffdc(xfr, &xfr->ffdc);
//					iic_ffdc_loc(&xfr->ffdc);
					IFLDe(1, "dma_submit_aio() = %d\n", rc);
					set_bit(IIC_XFR_DMA_COMPLETED,
							&xfr->flags);
					iic_abort_xfr(xfr);
					set_bit(IIC_XFR_ENG_COMPLETED, &xfr->flags);
...
//	iic_abort_xfr(xfr);
	...
	/* don't complete dma transfers unless dma driver has completed */
	if(!test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags) ||
	   test_bit(IIC_XFR_DMA_COMPLETED, &xfr->flags))
	{
		iic_xfr_complete(xfr);
...
int iic_boe_rescue_timeout(iic_eng_t *eng, iic_xfr_t* xfr)
{
	...
	if((fifo_cnt = stat & IIC_BOE_FIFO_COUNT) > 0) {
		...
		if(test_bit(IIC_XFR_DMA, &xfr->flags)) {
			/* DMA operation -- first step */
			xfr->bytes_xfrd += fifo_cnt;
		} else {
			if(test_bit(IIC_XFR_RD, &xfr->flags))
			{	/* read operation */
	...
	IFLDd(1, "test bit IIC_XFR_DMA_COMPLETED set, [%d]\n", xfr->bytes_xfrd);
	if(test_bit(IIC_XFR_DMA_COMPLETED, &xfr->flags)) {
		IFLDd(0, "bit IIC_XFR_DMA_COMPLETED set\n");
		retval = xfr->bytes_xfrd;
	}
