...
#define FSPTRACE_FILE_ID TFID_IIC_FSI
/*
...
static const char iic_fsi_version[] = "2.0011";

int iic_fsi_probe(struct device *dev);
int iic_fsi_remove(struct device *dev);
void iic_fsi_shutdown(struct device *dev);
int iic_fsi_suspend(struct device *dev, pm_message_t state);
int iic_fsi_resume(struct device *dev);
int iic_fsi_reset(void* vbase);
void iic_fsi_muxchg(struct device* dev);
	       ...
	       iic_ffdc_t** ffdc)
{
	...
	if(rc)
	{
		IFLDe(3, "eng[%08x]: fsi_readb_ffdc(%p)=%d\n", eng->id,
				addr, rc);
//		rc = iic_fsi_failure(eng, rc, ffdc);
	}
	return rc;
};
...
//		fsidevice_name(fdev, "iic.", bus_name, name, sizeof(name));
...
//			bb_put_minor(&iic_minors, minor);
		...
		if(test_bit(IIC_ENG_REMOVED, &eng->flags))
		{
			 ...
			 */
			rc = -ENODEV;
//			bb_put_minor(&iic_minors, MINOR(new_bus->devnum));
...
//			bb_put_minor(&iic_minors, MINOR(abusp->devnum));
...
typedef irqreturn_t fsi_irq_t (int, void *);
/*
...
//	struct fsi_addrmap mymap;
//	struct fsi_resource rp;
	iic_eng_t* eng = 0;
//	struct fsi_chipid chip_id; //needed to determine if nova dimm

	IENTER();

	/* retrieve the cfam chip id */
//	memset(&chip_id, 0, sizeof(chip_id));
//	fsi_getchipid(dp, &chip_id);

//	rc = fsi_get_resource(dp, FSI_DEVICE_IRQRES, &rp);
//	if(rc)
//	{
//		IFLDe(0, "fsi_get_resource failed\n");
//		rc = -EFAULT;
//		goto error;
//	}

//	fsi_addrinfo(dp, &mymap);
	...
	eng->id = 0x000B00B5;
//	eng->id = IIC_MK_ENG_ID(mymap.pa_link, mymap.pa_cfam, 
//				mymap.link, mymap.cfam, mymap.eng);
	IFLDi(1, "PROBE    eng[%08x]", eng->id);
	eng->ra = &fsi_reg_access;
//	eng->base = (unsigned long)mymap.addr;
	IFLDd(1, "vaddr=%#08lx\n", eng->base);
	eng->dev = dev;
...
//	rc = fsi_request_irq(rp.start,
//			 (fsi_irq_t *)eng->ops->int_handler,
//			 IRQF_DISABLED,
//			 "iic_fsi_dd",
//			 eng);
	if(rc)
	{
//		IFLDe(1, "request irq[%ld] failed: %d\n", rp.start, rc);
		goto error;
	}
...
//	eng->irq = rp.start;
	 ...
	 * BOE IOU/CFAM-S = 12.
	 */
//	IFLDi(2, "eng type:%08x chip id:%08x\n", dp->id.engine_type, chip_id.type);
...
error:
	if(rc)
	{
		...
		while(eng && eng->busses){
			iic_bus_t* temp = eng->busses;
			eng->busses = temp->next;
//			bb_put_minor(&iic_minors, MINOR(temp->devnum));
			iic_delete_bus(iic_fsi_class, temp);
		}
		if(eng)
		{
//			if(eng->irq)
//				fsi_free_irq(eng->irq, eng); //no rc
			if(eng->trace)
				kfree(eng->trace);
			kfree(eng);
		}
 ...
 *
 * The shutdown function is no longer called before this function in mcp5!  The
 * remove function must take care of quiescing the device now.
 */
int iic_fsi_remove(struct device* dev)
{
	int rc = 0;
...
//	state.event = FSI_LBUS_NO;
	...
	iic_fsi_suspend(dev, state); //ignore rc
...
//	int fsi_state = fsi_havelbus(fsidev);
	pm_message_t state;
       
//	state.event = (fsi_state && (fsi_state != -ENOLINK)) ?
//			 FSI_LBUS_YES : FSI_LBUS_NO;
...
int iic_fsi_suspend(struct device *dev, pm_message_t state)
{
	...
	IFLDi(2, "SUSPEND  eng[%08x] state[%d]\n", eng->id, state.event);
...
//		set_bit(IIC_NO_ACCESS, &eng->flags);
	...
	if (iic_eng_ops_is_vaild(eng->ops)) {
		...
		/* mask engine interrupts */
//		if(state.event == FSI_LBUS_YES) {
//			eng->ops->disable_int(eng, 0);
//		}
	}
...
//		if(rc) 
//		{
//			IFLDe(1, "fsi_disable_irq failed rc=%d\n", rc);		
//		}
	}
...
//	struct fsi_clocks clocks;
	...
	/* Run Basic Assurance Tests */
//	rc = eng->ops->run_bat(eng, &ffdc);
	if(rc)
	{
		goto error;
	}
...
//	}
	...
	/* Enable engine interrupt generation */
//	rc = eng->ops->enable_int(eng, &ffdc);
	if(rc)
	{
		IFLDe(1, "eng->ops->enable_int failed rc=%d\n", rc);
//		rc = fsi_disable_irq(eng->irq); 
		if(rc)
		{
			IFLDe(1, "fsi_disable_irq failed rc=%d\n", rc);
		}
...
//	rc = fsi_drv_register(&iic_fsi_driver);
	if(rc)
	{
...
//exit_debug_sysfs:
//	fsidrv_unregister(&iic_fsi_driver);
exit_drv_register:
	class_destroy(iic_fsi_class);
exit_class_create:
	unregister_chrdev_region(iic_devnum_start, IIC_FSI_MAX_DEVS);
exit_chrdev:
//	bb_del_minor(&iic_minors);
//	trace_unregister(iic_err_td);
exit_err_trc:
	IEXIT(rc);
//	trace_unregister(iic_td);
	return rc;
}
