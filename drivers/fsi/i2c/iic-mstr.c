...
#define FSPTRACE_FILE_ID TFID_IIC_MSTR
...
static const char iic_mstr_version[] = "2.0020";
...
struct file_operations iic_fops = {
	...
	.release = iic_release,
	.read = iic_read,
//	.aio_read = iic_aio_read,
//	.aio_write = iic_aio_write,
	.write = iic_write,
...
int iic_common_open(iic_client_t ** o_client, iic_bus_t * bus, int engine_num)
{
	...
	{
		/*
		 * No bus allocated means we were opened from a 'sideways'
		 * kernel call rather than via a sys call from user space.
		 * Get the appropriate bus from the opb driver.
		 */
		bus = iic_get_bus(engine_num, IIC_BUS_OPB);
		if(!bus)
		{
			ret = -ENOMEM;
			kfree(client);
		...
		client->flags |= IIC_CLIENT_SOURCE_KERN;
	}
	else
	{
...
int iic_abort_all(iic_eng_t* eng, iic_client_t* client, int status)
{
	...
	if(eng->cur_xfr && (!client || ( eng->cur_xfr->client == client)))
	{
		...
		if(!client)
		{
//			iic_fill_xfr_ffdc(cur_xfr, &cur_xfr->ffdc);
//			iic_ffdc_loc(&cur_xfr->ffdc);
		}
		iic_abort_xfr(cur_xfr);

		/* Don't call xfr_complete for dma transfers before dma
		 * operation has completed.  It could free up structures
		 * still being used by dma.
		 */
		if(!test_bit(IIC_XFR_DMA_SUBMITTED, &cur_xfr->flags) ||
		   test_bit(IIC_XFR_DMA_COMPLETED, &cur_xfr->flags))
		{
	...
	list_for_each_entry_safe(iterator, temp, &eng->xfrq, q_entry)
	{
		if(!client || (iterator->client == client))
		{
			...
			if(!client)
			{
//				iic_fill_xfr_ffdc(iterator, &iterator->ffdc);
//				iic_ffdc_loc(&iterator->ffdc);
			}
...
//		iic_ffdc_loc(&xfr->ffdc);
	...
	/* Get user space page mapping if we're not using dma or if we
	 * are using dma but there is a possibility of a retry.  (retried
	 * dma xfrs will be interrupt driven instead of dma driven and will
	 * require the page mapping).
	 */
...
{
	/* For really long timeout periods, use 1/2 timeout value */
//	if (((xfr->opts.xfr_opts.timeout >= 1000) && (xfr->size <= 128)) ||
//	    ((xfr->opts.xfr_opts.timeout >= 10000) && (xfr->size <= 8192)))
//		return (xfr->opts.xfr_opts.timeout / 2 );

	/* For short timeout and tiny transfer, leave 10 ms for retry */
//	if ((xfr->opts.xfr_opts.timeout >= 100) && (xfr->size <= 4))
//		return (xfr->retry_count ? 10 : xfr->opts.xfr_opts.timeout - 10);
...
void iic_abort_xfr(iic_xfr_t* xfr)
{
	...
	/* If this is a submitted but uncompleted DMA xfr, 
	 * notify DMA of the error so
	 * that our callback function is called and the
	 * xfr gets aborted/completed there.
	 */
	if(test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags) &&
	   !test_bit(IIC_XFR_DMA_COMPLETED, &xfr->flags))
	{
		IFLDd(1, "calling dma_notify(err) on xfr[%p]\n", xfr);
#ifdef IIC_DMA
		rc = dma_notify(&xfr->dma_key, DMA_ERR_EVENT);
		if(rc)
			BUG();
#endif
		goto exit;
	...
	else
	{
		...
		/* clean up dma if this was a dma xfr that was never submitted.
		 * This can happen if a read/write at offset xfr fails in the
		 * offset phase or if a xfr times out before even starting.
		 * submitted dma xfrs are cleaned up via the 
		 * dma_notify/callback method.
		 */
		if(test_bit(IIC_XFR_DMA, &xfr->flags) &&
		   !test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags))
		{
#ifdef IIC_DMA
			if((rc = dma_cancel(&xfr->dma_key)))
			{
				IFLDe(1, "dma_cancel() = %d\n", rc);
				BUG();
			}
#endif
...
void iic_timeout(unsigned long data)
{
	...
	IFLDd(1, "xfr->status[%d]\n", xfr->status);
	/* collect FFDC if not a dma transfer.  FFDC for dma xfrs is collected
	 * later in the iic dma callback function.
	 */
	if(!test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags))
	{
//		iic_fill_xfr_ffdc(xfr, &xfr->ffdc);
//		iic_ffdc_loc(&xfr->ffdc);
//		iic_fill_irpt_ffdc(eng, &xfr->ffdc);
	}
	...
	/* Don't call xfr_complete here for DMA transfers */
	if(test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags) &&
	   !test_bit(IIC_XFR_DMA_COMPLETED, &xfr->flags))
	{
		goto exit;
...
int iic_start_next_xfr(iic_eng_t* eng)
{
	...
	if(xfr)
	{
		...
		if(rc)
		{
			...
			/* don't complete dma transfers unless dma driver has
			 * completed
			 */
			if(!test_bit(IIC_XFR_DMA_SUBMITTED, &xfr->flags) ||
			   test_bit(IIC_XFR_DMA_COMPLETED, &xfr->flags))
...
exit:
	...
	if(ret < 0)
	{
//		iic_fill_client_ffdc(client, &ffdc);
//		ffdc->status = ret;
//		iic_ffdc_loc_1(&ffdc);
//		dd_ffdc_q_add(client->bus->ffdc_q, (dd_ffdc_t*)ffdc);
	}
