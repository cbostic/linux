...
#else
	return fsi_slave_read(dev->slave, addr, val, size);
#endif
}
...
// DEBUG ONLY vvvv
	rc = fsi_slave_read(slave, 0x800, &conf, sizeof(conf));
	if (rc) {
		printk("error reading 0x800\n");
		return 0;
	}
	printk("slave read of SMODE == 0x%08x\n", conf);
	rc = fsi_slave_read(slave, 0x808, &conf, sizeof(conf));
	if (rc) {
		printk("error reading 0x808\n");
		return 0;
	}
	printk("slave read of SISC == 0x%08x\n", conf);
	rc = fsi_slave_read(slave, 0x814, &conf, sizeof(conf));
	if (rc) {
		printk("error reading 0x814\n");
		return 0;
	}
	printk("slave read of SSTAT == 0x%08x\n", conf);
	rc = fsi_slave_read(slave, 0x28E0, &conf, sizeof(conf));
	if (rc) {
		printk("error reading 0x28E0\n");
		return 0;
	}
	printk("slave read of 0x28E0 == 0x%08x\n", conf);
	conf = 0xabcd1234;
	rc = fsi_slave_write(slave, 0x28E0, &conf, sizeof(conf));
	if (rc) {
		printk("error reading 0x28E0\n");
		return 0;
	}
